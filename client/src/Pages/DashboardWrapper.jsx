...
import Saved from "../components/Commands/Saved";
...
class DashboardWrapper extends Component {
  ...
  componentDidMount() {
    ...
    this.setState(
      { id: this.props.user._id, comBtnVisible: true, botClient: "notSet" },
      () => {
        console.log("Set dbw state with id", this.state.id);

        if (
          this.props.isLoggedIn &&
          this.props.user.botSettings !== undefined
        ) {
          // get user saved commands and update state
          helpers.getSaved(this.props.user._id).then(commandData => {
            this.setState({ userCommands: commandData.data }, () => {
              console.log(
                "set dbw state with user commands",
                this.state.userCommands
              );
              // then get sprite commands and update state
              helpers.getSprite(this.props.user.sprite_id).then(dbSprite => {
                this.setState(
                  { spriteCommands: dbSprite.data.commands },
                  () => {
                    console.log(
                      "set dbw state with sprite commands",
                      this.state.spriteCommands
                    );
                    this.loadBotClient();
                    window.showCanvas();
                  }
  ...
  addCommand = (newName, newMessage) => {
    helpers.postSaved(newName, newMessage, this.props.user._id).then(data => {
    ...
    return (
        <div className="row">
          <div className="col-sm-12 col-lg-6">
            <Dashboard
              channel={this.props.user.botSettings.channels[0].substring(1)}
              botClient={this.state.botClient}
            />
          </div>
          <div className="col-sm-12 col-lg-6">
          ...
          <div className="col-sm-12 col-lg-12">
            <Saved delCom={this.delCommand} comArray={this.state.userCommands} />
          </div>
        </div>
    );
  };
          ...
          <div className="container">
            {/* {viewCombtn ? (
              <button className="btn btn-success" onClick={this.showDashBtn}>
                <Link className="link" to={`${match.url}/commands`}>
                  List of commands
                </Link>
              </button>
            ) : (
